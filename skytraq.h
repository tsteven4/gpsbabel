...
protected:
  ...
  const char* opt_erase = nullptr;		/* erase after read? (0/1) */
  const char* opt_initbaud = nullptr;		/* baud rate used to init device */
  const char* opt_dlbaud = nullptr;		/* baud rate used for downloading tracks */
  const char* opt_read_at_once = nullptr;	/* number of sectors to read at once (Venus6 only) */
  const char* opt_first_sector = nullptr;	/* first sector to be read from the device (default: 0) */
  const char* opt_last_sector = nullptr;	/* last sector to be read from the device (default: smart read everything) */
  const char* opt_dump_file = nullptr;		/* dump raw data to this file (optional) */
  const char* opt_no_output = nullptr;		/* disable output? (0/1) */
  const char* opt_set_location = nullptr;	/* set if the "targetlocation" options was used */
  const char* opt_configure_logging = nullptr;
  const char* opt_gps_utc_offset = nullptr;
  const char* opt_gps_week_rollover = nullptr;
};
...
private:
  ...
  const char* opt_set_poi_home = nullptr;	/* set if a "poi" option was used */
  const char* opt_set_poi_car = nullptr;	/* set if a "poi" option was used */
  const char* opt_set_poi_boat = nullptr;	/* set if a "poi" option was used */
  const char* opt_set_poi_heart = nullptr;	/* set if a "poi" option was used */
  const char* opt_set_poi_bar = nullptr;	/* set if a "poi" option was used */
