...
#include <QtCore/QString>             // for QString, operator+, Qt::KeepEmptyParts
  ...
  while ((str = csv_lineparse(line, ",", "", lineno))) {
    ...
    switch (column) {
      ...
      case 2: {
        ...
        auto v = qstr.split('.', Qt::KeepEmptyParts);
      ...
      case 3: {
        ...
        auto v = qstr.split(QRegularExpression("[.:]"), Qt::KeepEmptyParts);
        if (v.size() == 3) {
          auto hour = v[0].toInt();
          auto min = v[1].toInt();
          auto sec = v[2].toInt();
