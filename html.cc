...
#include <ctime>                  // for localtime, tm, time_t

#include "defs.h"                 // for Waypoint, geocache_data, html_entitize, pretty_deg_format, rot13, strip_nastyhtml, waypt_disp_all, utf_string, METERS_TO_FEET, gpsbabel_testmode, gs_get_cachetype, gs_get_container, mkshort_del_handle, mkshort_from_wpt, mkshort_new_handle, setshort_length
...
HtmlFormat::html_disp(const Waypoint* wpt) const
{
  ...
  *file_out << "<td align=\"right\">";
  if (wpt->gc_data->terr) {
    ...
    *file_out << QStringLiteral("%1 / %2</p>")
              .arg(gs_get_cachetype(wpt->gc_data->type), gs_get_container(wpt->gc_data->container));
  }
  *file_out << "</td></tr>\n";
  ...
  *file_out << "<tr><td colspan=\"2\">";
  if (!wpt->gc_data->desc_short.utfstring.isEmpty()) {
    *file_out << QStringLiteral("<p class=\"gpsbabeldescshort\">%1</p>\n")
              .arg(strip_nastyhtml(wpt->gc_data->desc_short.utfstring));
  }
  if (!wpt->gc_data->desc_long.utfstring.isEmpty()) {
    *file_out << QStringLiteral("<p class=\"gpsbabeldesclong\">%1</p>\n")
  ...
  if (!wpt->gc_data->hint.isEmpty()) {
    ...
    *file_out << QStringLiteral("<p class=\"gpsbabelhint\"><strong>Hint:</strong> %1</p>\n").arg(hint);
  } else if (!wpt->notes.isEmpty() && (wpt->description.isEmpty() || wpt->notes != wpt->description)) {
    *file_out << QStringLiteral("<p class=\"gpsbabelnotes\">%1</p>\n").arg(wpt->notes);
  }
    ...
    if (fs_gpx && fs_gpx->tag) {
      ...
      while (curlog) {
        ...
        *file_out << "<p class=\"gpsbabellog\">\n";
        ...
        if (logpart) {
          *file_out << QStringLiteral("<span class=\"gpsbabellogtype\">%1</span> by ")
                    .arg(logpart->cdata);
        }

        logpart = xml_findfirst(curlog, "groundspeak:finder");
        if (logpart) {
          *file_out << QStringLiteral("<span class=\"gpsbabellogfinder\">%1</span> on ")
...
HtmlFormat::write()
{
  ...
  *file_out << " <meta http-equiv=\"Content-Type\" content=\"text/html;charset=utf-8\">\n";
  ...
  if (!gpsbabel_testmode()) {
    *file_out << QStringLiteral(" <meta name=\"Generator\" content=\"GPSBabel %1\">\n").arg(gpsbabel_version);
  }
  *file_out << " <title>GPSBabel HTML Output</title>\n";
  if (stylesheet) {
    *file_out << QStringLiteral(" <link rel=\"stylesheet\" type=\"text/css\" href=\"%1\">\n").arg(stylesheet);
  } else {
    *file_out << " <style>\n";
