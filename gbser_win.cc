...
#include <QtCore/Qt>                  // for CaseInsensitive, HANDLE
...
QString
fix_win_serial_name(const QString& comname)
{
  QString retval;
  constexpr char dev_prefix[] = R"(\\.\)";

  if (!gbser_is_serial(comname) ||
      ((comname.length() == 5) && (comname.at(4) == ':')) ||
      ((comname.length() == 4) && comname.startsWith("com", Qt::CaseInsensitive))
     ) {
    retval = comname;
  } else {
    if (comname.startsWith(dev_prefix)) {
      retval = comname;
    } else {
      retval = dev_prefix + comname;
    }
    if (retval.endsWith(':')) {
      retval.chop(1);
    }
...
void* gbser_init(const QString& port_name)
{
  ...
  const auto xname = fix_win_serial_name(port_name);

  gbser__db(2, "Translated port name: \"%s\"\n", qPrintable(xname));

  h->magic = MYMAGIC;

  HANDLE comport = CreateFileW(reinterpret_cast<const wchar_t*>(xname.utf16()),
...
{
  QRegularExpression re(R"(^(\\\\\.\\)?com\d+:?$)", QRegularExpression::CaseInsensitiveOption);
  return port_name.contains(re);
}
