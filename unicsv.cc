...
UnicsvFormat::rd_init(const QString& fname)
{
  ...
  unicsv_datum_idx = gt_lookup_datum_index(static_cast<const char*>(opt_datum), MYNAME);
...
UnicsvFormat::wr_init(const QString& fname)
{
  ...
  if (opt_grid != nullptr) {
    ...
    if (sscanf(opt_grid, "%d", &i)) {
      ...
      unicsv_grid_idx = gt_lookup_grid_type(static_cast<const char*>(opt_grid), MYNAME);
    }
  }
