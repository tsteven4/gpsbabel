...
NVector::NVector(const PVector& p_EB_E)
{
  ...
  double px2 = p_EB_E.getx() * p_EB_E.getx();
  double py2 = p_EB_E.gety() * p_EB_E.gety();
  double pz2 = p_EB_E.getz() * p_EB_E.getz();
  ...
  height = ((k+e2-1.0)/k) * sqrt(d*d+px2);
  x = sf * p_EB_E.getx();
  y = sf * sf2 * p_EB_E.gety();
  z = sf * sf2 * p_EB_E.getz();
}
