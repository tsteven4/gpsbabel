...
alpha(const void* a, const void* b)
{
  ...
  const vecs_t* const* ap = static_cast<const vecs_t *const*>(a);
  const vecs_t* const* bp = static_cast<const vecs_t *const*>(b);
...
sort_and_unify_vecs(int* ctp)
{
  ...
  svp = static_cast<vecs_t**>(xcalloc(vc, sizeof(style_vecs_t*)));
  ...
  for (style_vecs_t* svec = style_list; svec->name; svec++, i++)  {
    ...
    svp[i] = static_cast<vecs_t*>(xcalloc(1, sizeof** svp));
    svp[i]->name = svec->name;
    svp[i]->vec = static_cast<ff_vecs_t*>(xmalloc(sizeof(*svp[i]->vec)));
    ...
    *svp[i]->vec = *vec_list[0].vec; /* Interits xcsv opts */
    ...
    case 0:
    case wptdata:
      svp[i]->vec->cap[ff_cap_rw_wpt] = static_cast<ff_cap>(ff_cap_read | ff_cap_write);
      break;
    case trkdata:
      svp[i]->vec->cap[ff_cap_rw_trk] = static_cast<ff_cap>(ff_cap_read | ff_cap_write);
      break;
    case rtedata:
      svp[i]->vec->cap[ff_cap_rw_rte] = static_cast<ff_cap>(ff_cap_read | ff_cap_write);
      break;
    default:
      ;
    }
